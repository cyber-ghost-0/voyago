...
module.exports.token = async (req, res, next) => {
  ...
  if (refreshToken == null)
    return res
      .sendStatus(401)
      .json({ msg: "fault", err: "err:refreshToken is null" });
  if (!services.black_list.includes(refreshToken))
    return res
      .sendStatus(500)
      .json({ msg: "fault", err: "err:refreshToken is invalid" });
      console.log('___++',req.user_id,' ');
  jwt.verify(
    refreshToken,
...
module.exports.Logout = async (req, res, next) => {
  if (!services.black_list.includes(req.body.refresh_token)) {
    ...
    return res
      .status(500)
      .json({ data: {}, msg: "fault", err: "refresh token is not valid " });
  }
...
module.exports.forget_password = (req, res, next) => {
  ...
  return services
    .get_user_by_any(req.body.email, Admin, "email")
    .then((user) => {
      if (!user) {
        return res
          .status(500)
          .json({ msg: "fault", err: "This email isn't exist" });
      }
...
module.exports.check_verification_code = async (req, res, next) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ msg: "fault", err: "Time expired for the cod" });
  } else {
...
module.exports.reset_password = async (req, res, next) => {
  ...
  if (error) {
    return res
      .status(500)
      .send({ msg: "fault", err: error.details[0].message });
  }
